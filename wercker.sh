...
apt-get -qq install -y  xxd git flex libfl2 libomp-dev binutils-arm-linux-gnueabi g++-multilib gcc-multilib binutils-aarch64-linux-gnu python libisl-dev git ccache automake bc lzop bison gperf build-essential zip curl zlib1g-dev  g++-multilib  libxml2-utils bzip2 libbz2-dev libbz2-1.0 libghc-bzlib-dev squashfs-tools pngcrush schedtool dpkg-dev liblz4-tool make optipng openssl > /dev/null ;
export LOFASZ=$PWD;
